...
In Progress — items 1-3 completed (`fix/scoring-weights` commit `0de917a`,
`fix/bucket-admin-crud` commit `47ac134`, `feature/category-icon-upload` completed); items 4-5
not started.
...
3. **Category icon upload** (`feature/category-icon-upload`, in progress): `POST
   /api/categories/icons` (Customer, SVG only, mirrors avatar-upload pattern), wire the returned
   URL into `CreateCustomCategoryRequest`/`CategoryService.CreateCustomCategoryAsync`.
...
- Live verification uses a locally-inserted test admin (`khoi_test_admin`, local DB only, not the
  deployed backend) since the seeded default admin credentials didn't match this local DB.
