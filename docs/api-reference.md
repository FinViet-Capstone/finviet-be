...
**ExtractedTransactionItem**: `{ amount, type, merchant?, description?, transactionDate, amountConfidence?, merchantConfidence?, transactionDateConfidence?, categoryId?, categoryName?, confidence?, rawFields? }`. The three field-confidence values are populated by receipt OCR only; `confidence` remains the independent category-suggestion confidence. `rawFields: { header, value }[]` is `/csv`-only (always `null` for `/sms`/`/photo`) - one pair per header-row column, in file order, not just columns mapped to a normalized field above; a data row with more raw values than the header row has columns for has those extra trailing values dropped (no header text exists to pair them with). Also `null` for CSV rows parsed via the header-less legacy positional layout, for the same reason. A response can mix rows with and without it.
...
**Business logic**: `.csv` parsed via `CsvHelper` (delimiter sniffed from the first line: `,`/`;`/tab, whichever occurs most); `.xlsx`/`.xls` via `ExcelDataReader`. Both feed the same row list into `BankStatementRowParser`, which resolves columns two ways: **(1) by header name** (Vietnamese/English aliases for Date/Description/Amount or Debit+Credit, optionally Correspondent) when a recognizable header row is found — this is what a simple `Ngay,Mo ta,So tien` export needs, with a single signed Amount column (negative = expense, positive = income); **(2) fixed positional fallback** when no header is recognized, preserving the original full bank-statement layout: col 1 = STT (row detector — non-numeric rows skipped silently), col 2 = date, col 5 = debit, col 6 = credit, col 11 = description, col 13 = correspondent/beneficiary. Amount parsing strips `VND`/`VNĐ`/`đ`/`₫` and detects Vietnamese-locale (`.`-thousands/`,`-decimal) vs. invariant (`,`-thousands/`.`-decimal) formatting. Date parsing accepts `dd/MM/yyyy[ HH:mm[:ss]]` and `yyyy-MM-dd`/`yyyy/MM/dd` (plus a culture-invariant fallback). When the response comes back with zero rows, `message` explains the expected format instead of a generic success string. `maxRows` **truncates after parsing** (`Take(maxRows)`) — `totalScanned`/`skipped` still reflect the full file. No duplicate-detection logic exists (consistent with parse-only design).

### POST `/photo`
**Validation**: file required/non-empty → 400 "Vui lòng chọn ảnh hóa đơn để trích xuất."; size `> 8 MB` (`MaxPhotoFileBytes`, exact) → 400 "Ảnh quá lớn (tối đa 8 MB)."; extension must be `.jpg`/`.jpeg`/`.png`/`.heic`/`.heif`/`.webp` → 400 otherwise. The API maps that validated extension to a canonical Gemini-supported MIME type rather than trusting the client-supplied multipart content type.
**Business logic**: `IReceiptOcrService` → `GeminiReceiptOcrService` (wired 2026-08-18, commit `aff76cc`), reusing the same Gemini model/key as score/report/chat — reads the photo and returns amount/merchant/description/transactionDate plus field-level confidence, or `null` if it isn't a readable receipt. The prompt handles rotated receipts and distinguishes the final total/payable amount from item prices, tendered cash and change. Confidence is clamped to `[0,1]`; an unreadable/missing date uses the existing current-date fallback with `transactionDateConfidence=0` so the confirmation UI flags it instead of presenting it as certain. The returned row then runs through the same rule-then-AI category suggestion as SMS/CSV extraction (`ITransactionExtractService.CategorizeItemAsync`). `UnconfiguredReceiptOcrService` (previously always throwing `IntegrationUnavailableException` 503 `ocr_not_configured`) is kept in the codebase, unregistered, as a fallback if OCR needs to be disabled again.
